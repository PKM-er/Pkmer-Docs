---
uid: 7913
title: 快速编辑的魔法——OB内的文本指令
tags: [obsidian, 插件, 教程, 工作流]
description: 快速编辑的魔法——OB内的文本指令
author: Moy
type: other
draft: false
editable: false
modified: 20260106234946
forum_url: https://forum.pkmer.net/t/7117
---
...
> - 作者: Moy
> - 原始链接: [快速编辑的魔法——OB内的文本指令](https://forum.pkmer.net/t/7117)
...
通过它，我们能进一步拓宽「快捷键」的限制——再也不用担心快捷键太多记不住了，因为现在可以直接<mark>通过特定文字来执行我们想要的操作</mark>！

说的好听，用起来是什么样子的呢？
这里举两个例子。

### 一、快速获取浏览器网址
![240627_实现OB内的文本指令-img-240630_014745.gif](upload://jqEbKdBAKyNKr5jjkl6wZNmB0RN.gif)

在上图中，只要在 OB 内输入 `lnk` 并空格，就会调用 Quicker 执行动作[^1]，从当前浏览器获取到标签页并粘贴进来。

### 二、语义化插入模板
常用 Templater 的朋友应该都会有一个烦恼：模板太多了！
使用文本指令之后，我们可以直接把模板对应到文字：

![240627_实现OB内的文本指令-img-240630_234228.gif](upload://uLLrG7Mc1GNVbWumTgE63XTbkXk.gif)

如上图，只需要输入 `代码模板 ` 几个字，就会自动调用预先配置好的模板。
（如果担心误触也可以改成类似 `tp代码` 之类的触发关键字）

这种方式比起「打开插入模板的选择菜单，找到需要的模板，插入模板」是不是方便很多呢？

___

在 Obsidian 内，我们可以用插件 **Latex Suite** [^2] 实现上述的功能。

> [!NOTE] 文本替换插件
> Latex Suite 其实是个文本替换插件，类似的还有 Easy Typing，两者都可以在输入特定的文本之后将它转换成其他文本。
> 但 JS 代码的执行功能是 LatexSuite 插件独有的，也是它最大的亮点。

## 定义文本指令 🪄
在 LatexSuite 的设置中，用 json 来定义替换规则。

![240627_实现OB内的文本指令-img-240630_020629.jpg](https://cdn.pkmer.cn/original/1X/8839144e31b4eda114947814231114cc24bb0264.jpeg)
...
![240627_实现OB内的文本指令-img-240630_032452.gif](upload://iY15gjVIAAuhcpYQBkw3nJkS3Zc.gif)


___

除了填写固定的替换文本，还可以填写 Javascript 代码，以添加动态的文本。
例如：
```js
...
![240627_实现OB内的文本指令-img-240630_021218.gif](upload://6rI4mtYGZpxstiyZ61iAMdtxyny.gif)

> 关于函数运行，更多介绍可以看插件的说明：[Function snippets](https://github.com/artisticat1/obsidian-latex-suite/blob/main/DOCS.md#function-snippets)

> 🦊 我原先其实都是用的 Easy Typing 插件做文本替换（很好用！），直到有一天想要插入像这样的日期格式发现办不到……于是才辗转找到了 Latex Suite。
...
![240627_实现OB内的文本指令-img-240627_194029.webp](https://cdn.pkmer.cn/original/1X/a532b02bb146292f4cf42697ca47c5d8c636d482.png)

选中命令的 ID 并回车，就能看到这个命令的详细信息：
![240627_实现OB内的文本指令-img-240627_194149.webp](https://cdn.pkmer.cn/original/1X/4c659b08c8a39e1a1e5b1815c4f486bcea57118a.png)
...
![240627_实现OB内的文本指令-img-240630_021735.webp](https://cdn.pkmer.cn/original/1X/10cc35fffa751f87ca33eae99cd55004f2db2dc6.webp)
> 这里填写的内容和你在系统「运行」中执行的命令等同，所以也可以按 <kbd>Win+R</kbd> 来测试命令（在命令行执行也一样）
> ![240627_实现OB内的文本指令-img-240630_022056.webp](https://cdn.pkmer.cn/original/1X/5f2e144085d5b4742a54b5df13aa8d1abe41e256.webp)
...
![240627_实现OB内的文本指令-img-240630_023045.webp](https://cdn.pkmer.cn/original/1X/cdfa63d7eae02b84cef50a4aa88625ba92bf0444.webp)

我在 SC 命令的 **Variables** 中将 `selection` 变量的默认值设置为 `NO-SELECTION`：
![240627_实现OB内的文本指令-img-240630_023651.webp](https://cdn.pkmer.cn/original/1X/c600a6180e720b7c367855d8cfac8656babefc3d.webp)

这样 Quicker 动作就可以根据传入的参数，来决定粘贴的方式：
- 如果有选中文本，就把链接粘贴进当前文本
- 否则，就粘贴完整的 Markdown 格式链接

![240627_实现OB内的文本指令-img-240630_023941.gif](upload://jJTea7KSz08Z1zNJDGAZTpwugHX.gif)
...
![240627_实现OB内的文本指令-img-240630_024345.jpg](https://cdn.pkmer.cn/original/1X/5d101465097a4b9b4a506a40cd8a03a1f878e377.jpeg)
...
[^1]: 我自己写的用来在 OB 里快速粘贴的动作：[Ob粘贴 - by Moy - 动作信息 - Quicker](https://getquicker.net/Sharedaction?code=b259db0a-db2b-4c84-658e-08dc85dec23e) ，以及获取标签页的动作：[获取标签页 - by Moy - 动作信息 - Quicker](https://getquicker.net/Sharedaction?code=cad4688c-ec09-4aa5-0613-08d838181f56)
[^2]: [artisticat1/obsidian-latex-suite](https://github.com/artisticat1/obsidian-latex-suite)
