...
> 设置 Canvas 缩略图后卡片的占位符大小随窗口变化，这里控制放大倍数，让占位符更加明显。
...
> 原生的 Canvas 卡片背景颜色会阻碍笔记的阅读，可以通过该 CSS 片段取消背景颜色。
