---
uid: 20230715051519
title: Obsidian 插件：一个可以定制化的粘贴插件，可以在粘贴到 Obsidian 之前处理文本。比如 csv 转 md 表格，网页 html 转 md 表格
tags: [粘贴, 自定义, 表格, 文本处理]
description: 
author: Nathaniel
type: basic
draft: false
editable: false
modified: 20230715063643
---

# Obsidian 插件：一个可以定制化的粘贴插件，可以在粘贴到 Obsidian 之前处理文本。比如 csv 转 md 表格，网页 html 转 md 表格
...
![[Pasted image 20230715053646.png]]
...
1. 配置脚本储存目录
进入 插件选项（option）
![[Pasted image 20230715061414.png]]
设置一个目录路径，比如当前是 `attachments/adpaste-scripts` 表示根目录下的 `attachment` 文件夹下的 `adpaste-scripts` 文件夹（前提需要有这个文件夹）
![[Pasted image 20230715061432.png]]
在此文件夹下创建一个后缀为 `.js` 的文件（比如 `my-script.js`），并在其中编写代码（可以直接将**源码**中内容复制）
![[Pasted image 20230715061738.png]]
2. 重启 obsidian
此时，可以在快捷键设置中中查看添加的命令
![[Pasted image 20230715061937.png]]
![[Pasted image 20230715061949.png]]
3. 分配一个快捷键（可选）
4. 粘贴之前执行命令
![[Pasted image 20230715062103.png]]
效果
![[Pasted image 20230715062135.png]]
源码：
...
![[Pasted image 20230715060823.png]]

复制它

![[Pasted image 20230715060834.png]]

然后转到 Obsidian 中，在粘贴之前，调用命令（或者按下设定好的快捷键）触发执行

![[Pasted image 20230715060840.png]]

转换后的效果：

![[Pasted image 20230715060746.png]]

对比使用插件内置的**默认转换**的效果

![[Pasted image 20230715060722.png]]

除了网页上的表格，CSV 中的表格也可以在粘贴前转换（前提是简单的，非合并的表格）

CSV( Excel ) 中的表格

![[Pasted image 20230715062958.png]]

处理后的效果

![[Pasted image 20230715063026.png]]

![[Pasted image 20230715063039.png]]
