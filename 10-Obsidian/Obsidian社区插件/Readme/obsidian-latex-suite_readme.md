...
> - 插件名称：Latex Suite
> - 插件作者：artisticat1
> - 插件说明：通过片段、文本扩展和编辑增强功能，使LaTeX数学排版速度接近手写速度。
...
通过片段、文本扩展和编辑增强功能，使LaTeX数学排版速度接近手写速度。
...
Obsidian的一个插件，旨在使排版LaTeX数学公式的速度与手写一样快。
...
该插件的主要功能是**片段**，通过快捷方式和文本扩展帮助您更快地编写LaTeX！例如，输入
...
通过隐藏LaTeX代码，使您的方程更易读，而不是以漂亮的格式呈现。
...
- `priority`（可选）：此代码片段的优先级。默认为0。优先级较高的代码片段先执行。可以为负数。
...
对于名称较短（2-3个字符）的希腊字母，只需输入其名称，例如 "pi" → "\\pi"
