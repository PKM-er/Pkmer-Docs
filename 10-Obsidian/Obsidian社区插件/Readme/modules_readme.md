...
> - 插件名称：Modules
> - 插件作者：polyipseity
> - 插件说明：从保险库和互联网加载JavaScript和相关语言（如TypeScript）模块。
...
从Vault和互联网加载JavaScript和相关语言（如TypeScript）模块。
...
- 在所有平台上从保险库和互联网加载JavaScript和TypeScript模块。
- 无需配置。
- 解析相对路径、保险库路径、Markdown链接、wikilinks和外部链接。
- 将Markdown文件加载为代码。
- 支持在模块内使用其他模块。
- 加载CommonJS (`module.exports`)和ES模块 (`export`)。
- 支持CommonJS模块的循环依赖。
- 可配置的require名称。
- 添加用于调试的源映射。
- 支持Dataview和Templater等流行插件。
## 安装
...
mod.eat(2 * mod.pi)
```
- 要创建一个模块，请创建一个JavaScript或相关语言文件，或者一个带有JavaScript或相关语言代码块的Markdown文件。
	...
	- 对于Markdown文件，可以在设置中配置加载的代码块语言。
	- 对于非JavaScript语言，请确保模块文件具有正确的文件扩展名（也适用于`.xxx.md`），或者在前面添加以下元数据：
```TypeScript
// { "language": "TypeScript" }
...
```
- 完整的API可以从[`sources/@types/obsidian-modules.ts`](sources/%40types/obsidian-modules.ts)获得。
贡献
