...
- 简单易用但功能丰富的配置选项
- 排序配置直接存储在您的笔记正文中
  - 在YAML中使用专用的`sorting-spec:`键
- 未设置自定义顺序的文件夹仍然按照标准的Obsidian排序
...
点击Ribbon图标可以在启用和挂起状态之间切换插件。

在大屏设备（台式机、笔记本电脑和iPad等平板电脑）上的Ribbon图标状态：
...
- ![Static icon](./docs/icons/icon-mobile-initial.png) （仅适用于大屏移动设备如iPad）。插件已启用，但未应用自定义排序。
...
- Obsidian命令面板很容易使用（在小屏移动设备上向下滑动手势），可以通过命令快速控制插件：sort-on和sort-off。这可能比导航到并展开功能区更容易。
...
插件可以和应该从官方Obsidian社区插件列表（https://obsidian.md/plugins）或直接在Obsidian应用程序中安装。
...
这个功能不依赖于Custom Sorting插件。
同时我在这里提到它是因为它是与[@replete](https://github.com/replete)的讨论的一个副作用。
我们考虑直接在插件中支持分隔符。最终，这归结为一个非常简洁和智能的基于CSS片段的解决方案，与插件无关。去看看，在Obsidian的CSS片段中复制它，享受更加分组的外观。
