...
一旦您标记了至少一个笔记，请创建一个新的笔记，并添加一个新的Markdown代码块，使用三个反引号标记为`aat-vertical`，并将时间轴的名称添加为其内容。
...
该插件提供了3个主要设置，以帮助实现您所需的时间格式。
在深入了解太多细节之前，请确保您熟悉正则表达式和其中的命名捕获组。如果不是这种情况，fireship.io有一个[适合初学者的优秀视频](https://www.youtube.com/watch?v=sXQxhojSdZM)可以开始您的学习之旅。
...
此设置应直接基于先前创建的正则表达式。在默认正则表达式中，我们有3个主要的标记。`year` `month` 和 `day`。您可以在命名捕获组中找到它们。
...
根据obsidians的[非官方文档](https://marcus.se.net/obsidian-plugin-docs/getting-started)：
