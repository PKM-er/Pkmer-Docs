...
> - 插件分类：['文件管理', '标签', 'obsidian插件', 'readme']
...
将TagFolder放置在左侧窗格，并在每次启动Obsidian时激活它。
...
禁用缩小范围
TagFolder通过收集在同一笔记中使用的标签组合来创建文件夹结构，以便我们更容易找到笔记。
...
点击标签时，我们可以在TagFolder中搜索标签，而不是打开默认的搜索窗格。
使用控制键和Shift键，我们可以从搜索条件中删除标签，或者将其排除在外。
...
合并冗余组合
当启用此功能时，如果没有中间项，a/b和b/a将合并为a/b。
