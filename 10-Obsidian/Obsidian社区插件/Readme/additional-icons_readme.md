...
我倾向于使用来自[metadata menu](https://mdelobelle.github.io/metadatamenu/)的许多`fileClass`，并经常使用`icon`字段根据它们的类型为文件提供单独的图标。我发现自己想要使用与Obsidian默认的Lucide套件中提供的图标不同的图标。现在你可以使用诸如`ra-axe`、`gi-dna`和`wi-tornado`之类的东西来装饰你的文件。
# 许可证

该存储库根据BSD 3-Clause许可证获得许可。与构建版本捆绑在一起的字体（未与源代码一起分发）根据上述许可获得许可。
# 资助
