...
### 调整边距箭头
通过在起始标识符后附加"`|x-pos`"来调整边距箭头的x位置，其中`x-pos`是0到30之间的数字，例如`{my-arrow|blue|10}`。
...
- `{arrow-identifier|color|opacity|type|x-position|arrowhead}` 来表示箭头的起始点，其中：
    ...
    - `color` 是CSS表示法中的任何有效颜色，例如 `red`, `#ff0000`
    - `opacity`（可选）是介于0和1之间的浮点数，例如 `0.6`。默认为1。
    - `type`（可选）是 "`diagonal`" 或 "`margin`" 中的一个。默认为 "`margin`"。
    - `x-position`（可选）是介于0和30之间的整数。设置边距箭头的x位置。默认为0。
