...
![[Pasted image 20250624095701.png]]

然而，原生样式存在几个痛点：

- 多行标题内部行距过大，与标题间距混淆 ![[image.png|多行标题与其他标题难以区分]]
- 层级结构视觉区分度不足
- 交互反馈不够优雅
- 与主题配色缺乏协调
...
- 缩小时产生多个滑动条； ![[image-2.png|缩小插件高度，发现文档中出现两个滑动条]]
- 侧边父节点展开的引导线消失;
...
- **效果对比**：
  - 优化前：多行标题内部行距与标题间距相同，难以区分内容边界 ![[image-4.png|优化前效果]]
  - 优化后：多行标题内部紧凑连贯；两个标题间距清晰可辨 ![[image-5.png|优化后效果]]
...
- **四重状态指示**：
  ...
  - 顶部导航栏：进度条与主题色彩同步
![[image-7.png|大纲标题颜色同步]]

![[image-10.png|导航栏进度条颜色同步]]
...
- **边界优化**：修复了父节点引导线消失的问题
![[image-8.png|恢复父节点展开后的引导线]]

![[image-9.png|也可以重新使用引导线颜色]]
